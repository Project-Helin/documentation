...
Für den Server Teil wurde das Play Framework ausgewählt, denn zum grössten Teil bestehen die Komponenten aus CRUD Operationen.
Die Gründe für den Wahl waren:
\begin{itemize}
    \item Play kam bereits mit Integration für Datenbanken
    \item Mit Play sollte es einfach und schnell möglich sein eine Web Applikation zu erstellen, wie dies etwa aus anderen Frameworks wie Ruby on Ralis gewohnt ist.
    \item Spring als Alternative war allen Teammitgliedern bestens bekannt, und mit Play gab es die Möglichkeit ein neues Framework ausführlich in ein langfristiges Projekt einzusetzten
\end{itemize}

Während der Implementierung stellten sich mehrere Schwächen von Play heraus. 
Im folgenden werden anhand einiger Punkte erklärt, warum wir Play nicht für ein weiteres Projekt im Betracht ziehen.
...
Deshalb wurde auch bei uns alle CRUD Seiten von Hand geschrieben. 
...
Neben der Dokumentation war auch die Fehlende Community ein störend Faktor.
Im Internet existieren viele Antworten, doch aufgrund der verschiedene API sind nicht alle aktuell . 
Es musst meist direkt bei den Entwicklern nachgefragt ( z.b. IRC Chat ) oder über Github Issue \cite{github-ticket} die fehlenden Informationen geholt werden.
...
Zum einen liegt es sicherlich daran, dass neue Funktionen hinzukommen und alte Komponenten abgebaut werden müssen. 
Doch der Hauptgrund ist sicherlich, dass Play ein Framework für zwei Sprachen, Java und Scala ist. 
...
So kommt es dass, der Fokus auf Scala gelegt wir wird Java für Play die Sprache zweiter Klasse ist.


\subsubsection{Handling der Kommunikation mit Drohnen}
...
Wie sicher ist die Kommunikation mit dem Server? Kann allenfalls eine Drohne gekapert und falsche Koordinaten zugeschickt werden?
\\
Das CustomerApp registriert sich über einen HTTPS Request beim Server. Bei erfolgreicher Anmeldung werden neben einem Token auch die Anmeldedaten für RabbitMQ mitgeliefert.

Damit kann sich die App mit dem Messaging Broker verbinden, auch diese Verbindung läuft verschlüsselt.
Für die darauf folgende Kommunikation mit dem Server, werden zwei Queues erzeugt: 
\begin{itemize}
	 \item \{token\}-Drone-To-Server
	 \item  \{token\}-Server-To-Drone
\end{itemize}

Beim Token handelt es sich um den vom Server gesendeten UUID.
Über diesen Kanal werden JSON Messages ausgetauscht. Ein Angreifer müsste den Token herausfinden, um mit der Drohne zu kommunizieren. Mit einer Brutforce Methode ist mit $2^{122}$ Möglichkeiten ein Treffer auszuschliessen. 
Das auslesen des Tokens ist auch ausgeschlossen, da die erstmalige Verbindung verschlüsselt ist. Somit ist für uns die Kommunikation mit der Drohne ausreichend gesichert.
