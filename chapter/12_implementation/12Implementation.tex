...
% Abschnit oben:
% - wieso funktionale? sind ja nur nicht-funktionale Anforderung
...
% oben: 
% - unklar was genau heist, dass "queue eingestzt wird, ohne Datenpersistenz"?
% - 
...
% - äh, backoff oder zeitbasiert ( Was heisst das, ist ja auch zeitbasiert) - verhindert ja nicht, dass alle clients gleichzeitig auf server verbinden
	\item{\textbf{Security:} 
...
% Security:
% - etwas ausführlich beschreiben, was wir letztens besprochen haben ( z.b. queue mit token namen, token ist nicht erratbar)
% - aber das gehört eigentlich nicht da? ( es geht ja hier um messaging, und verbindugnsabbruch, wieso kommt security hier?)
...
Es kann somit gesagt werden, dass Android etwa 4s benötigt um eine stabile GSM oder WLAN Verbindung wieder aufzubauen.

% ausführlich beschreibne, was du getestet hast
% - internet abbruch simuliert -> was ist passeirt (sind messagins angekommen ? oder exception geworfen )
% - app geht in background -> was ist passiert, sind messages angekommen
% - gehen message verloren, oder neugeschickt
% - verbindungsabbruch -> 
