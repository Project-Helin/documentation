...
\end{itemize}
Für Verwendungszwecke ohne den Bedarf eines User Interfaces kann man sich allenfalls auch überlegen andere Komponenten zu verwenden. Wir haben zwei Optionen genauer angeschaut und bewertet:
\begin{itemize}
...
\end{itemize}
Aus unserer Sicht stellen die zwei Embedded Systeme durchaus eine alternative dar. Leider verschwindet aber der Komfort die Drohne mittels dem GUI auf der Onboard App zu überwachen . 
Beide Alternativen müssen zusätzlich über das Boardstromnetz der Drohne betrieben werden, und benötigen zusätzliche Spannungsregler um die gewünschte Betriebsspannung der Hardware zu gewährleisten. 
...
Da wir uns entschieden haben, die Lieferung abzuwerfen, und die Drohne nicht zu landen, musste sichergestellt werden, dass der Aufprall sich in einem niedrigen Ausmass ereignete. Der Ausmass durfte das Produkt nicht beschädigen.
\\
...
\end{split}
\end{equation}
Nun musste ein Fallschirm dimensioniert werden, der eine Flasche mit 500g Inhalt sicher zu Boden bringt. Da dies alles Annäherungen sind, nehmen wir der einfachheithalber an, dass die Flasche mit Inhalt 500g schwer ist und der Fallschirm die Form einer Kugelkalotte. 
%Todo Skizze
\begin{equation}
\begin{split}
F_{Brems} &= F_{G} \\
...
\end{split}
\end{equation}
Für die Werte $\rho$, $c_w$ wurden die Standardwerte genommen. Bei der Luftdichte $\rho$ entspricht das $1.225 \frac{kg}{m^3}$ und für $c_w$ den Literaturwert für die konkave Seite der Halbkugelschale $1.33$. Zusätzlich, lässt sich der Durchmesser über die Öffnungsfläche berechnen $A = \pi \cdot r^2$.
\begin{equation}
\begin{split}
...
d &= 2 \cdot \sqrt{\frac{2 \cdot 0.5kg \cdot  9.81\frac{m}{s^2}}{1.225 \frac{kg}{m^3} \cdot 1.33 \cdot 4.5 \frac{m}{s} \cdot \pi}} \\
d &= 1.30m \\
\end{split}
\end{equation}
Der Durchmesser des Prototypenfallschirms muss somit etwa 1.30m betragen.
%TODO MARCEL: 2 - 3 Sätze zum Bau und noch eine foto? :)
...
Grundsätzlich können, über das von uns verwendete \Gls{MAVLink} Protokoll, alle Arten von Drohnen angesteuert werden. Beispielsweise bietet die Autopilot-Software 'ArduPilot' Firmware-Varianten für Multicopter, Flugzeuge, Fahrzeuge an, wie dies in der Abbildung \ref{fig:arduScreenshot} dargestellt. \\
\begin{figure}[H]
\centering
...
Uplift  Aero beispielsweise war ein Startup, dass versucht hat, Hilfsgüter mit Hilfe von Drohnen von der Türkei aus nach Syrien zu fliegen. Sie haben für ihre Flugzeuge (Grafik \ref{fig:uplift}) die selben Flugcontroller wie wir verwendet und mussten lediglich die passende Firmware aufspielen. Dies beweist wie flexibel und vielseitig die von uns eingesetzte Hardware verwendet werden kann.
