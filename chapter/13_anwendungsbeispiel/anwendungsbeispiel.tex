...
Beide Alternativen bringen zusätzlich mit sich, dass sie externe Stromversorgung benötigen. Somit muss auf die Bordstromversorgung zurückgegriffen werden und das \Gls{BEC} für die Servo-Stromversorgung verwendet werden muss um eine stabile Betriebsspannung zu generieren. \\
...
\end{split}
\end{equation}
Für die Werte $\rho$, $c_w$ wurden die Standardwerte genommen. Bei der Luftdichte $\rho$ entspricht das $1.225 \frac{kg}{m^3}$ und für $c_w$ den Literaturwert für die konkave Seite der Halbkugelschale $1.33$. Herr Prof. Dr. F. Müller hat uns darauf hingewiesen, dass bei seinen Raketenversuche, er einen $c_w$-Wert von 1.8 als gute Annäherung verwendet. Aus diesem Grund haben wir diesen für die weitere Berechnung eingesetzt. Zusätzlich, lässt sich der Durchmesser über die Öffnungsfläche berechnen $A = \pi \cdot r^2$.
\begin{equation}
\begin{split}
